...
class MZV_LB_CSS_Mode {
	...
	public static function get_inline_css(): string {
		...
		return <<<CSS
/* This Little Lightbox of Mine v2.5.0 — CSS-Only Mode */
...
.llb-wrap{--llb-trigger-icon-size:28px;position:relative;display:inline-block;cursor:zoom-in}
.llb-wrap img{display:block}
.llb-icon-jumbo{--llb-trigger-icon-size:56px}
.llb-icon-super{--llb-trigger-icon-size:84px}
...
.llb-hover::after{content:'';width:var(--llb-trigger-icon-size);height:var(--llb-trigger-icon-size);background:url("{$magnifier_svg}") center/contain no-repeat;filter:drop-shadow(0 1px 2px rgba(0,0,0,.5))}
.llb-wrap:hover .llb-hover{opacity:1}
.llb-mobile-hint{position:absolute;bottom:8px;right:8px;width:calc(var(--llb-trigger-icon-size) * .79);height:calc(var(--llb-trigger-icon-size) * .79);background:url("{$magnifier_svg}") center/contain no-repeat;opacity:.6;filter:drop-shadow(0 1px 2px rgba(0,0,0,.6));pointer-events:none}
@media(hover:hover){.llb-mobile-hint{display:none}.llb-icon-always .llb-hover{inset:auto;right:8px;bottom:8px;width:var(--llb-trigger-icon-size);height:var(--llb-trigger-icon-size);background:none;opacity:.8}.llb-icon-always:hover .llb-hover{opacity:1}}
...
CSS;
	 ...
	 */
	private static function get_wrap_classes( array $opts ): string {
		$classes = [ 'llb-wrap' ];
